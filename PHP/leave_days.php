<?php
	...
	function approved_leave_days($link, $eid, $lid)
	{
		// total no. of days of this leave are used by this employee (approved or requested leaves)

		$query = "SELECT SUM(days) AS total_days FROM (SELECT DATEDIFF(end_date, start_date) + 1 AS days FROM leave_request WHERE eid = $eid AND lid = $lid AND (mg2_consent = 'A')) AS days_table";
